...
/* ---------- BACKGROUND IMAGE (plexus network reference) ---------- */
.bg-video-wrap{
...
def _load_bg_image_b64():
    image_path = Path(__file__).resolve().parent / "assets" / "bg-network.jpg"
...
st.markdown(f"""
<div class="bg-video-wrap" style="background-image: url('data:image/jpeg;base64,{_load_bg_image_b64()}');">
...
# resolves out of scrambled characters, deliberately slowed down so the
# decode itself is easy to watch rather than a blink-and-miss-it flicker).
# ============================================================
def _scramble_trigger_html(nonce: str) -> str:
    # The nonce forces the iframe's srcdoc to change on every Streamlit
    # rerun, which forces the browser to reload it and re-run the script —
    # otherwise an unchanged iframe is left alone and newly-appeared
    # headlines (e.g. a fresh answer card) would never get scrambled.
    return f"""
    <!-- SCRAMBLE_TRIGGER_MARKER {nonce} -->
    <!DOCTYPE html>
    <html><head></head><body>
    <script>
    (function(){{
      try {{
        ...
        var duration = 850;    // noticeable, but quick enough not to drag
        var frameMs = 35;
        ...
        els.forEach(function(el, idx){{
          ...
          setTimeout(function(){{
            ...
            var totalSteps = Math.max(24, Math.round(duration / frameMs));
            var step = 0;
            function frame(){{
              step++;
              var revealed = Math.floor((step / totalSteps) * len);
              var out = '';
              for (var i = 0; i < len; i++){{
                var ch = original[i];
                if (ch === ' ' || ch === '\\n' || ch === '\\t'){{ out += ch; continue; }}
              ...
              el.textContent = out;
              if (step < totalSteps){{ setTimeout(frame, frameMs); }}
...
if st.session_state.app_stage == "loading":
    ...
    """, unsafe_allow_html=True)
    components.html(_scramble_trigger_html(str(time.time())), height=1, scrolling=False)
    time.sleep(1.4)
    ...
    # Fires once per rerun, after every headline on this stage already
    # exists in the DOM — scrambles in anything not already revealed
    # (a fresh answer card, an updated stat count, etc).
    components.html(_scramble_trigger_html(str(time.time())), height=1, scrolling=False)
