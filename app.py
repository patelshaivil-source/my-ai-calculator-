...
.stat-glass-orange{ background: linear-gradient(160deg, rgba(255,122,61,0.92), rgba(255,90,31,0.78) 55%, rgba(122,31,92,0.55)); border-color: rgba(255,154,82,0.35); }
.stat-glass-white{ background: linear-gradient(160deg, var(--blue-b) 0%, var(--blue-a) 100%); border-color: rgba(120,160,255,0.32); }
.stat-glass-black{ background: linear-gradient(160deg, var(--violet-b) 0%, var(--violet-a) 100%); border-color: rgba(180,140,255,0.30); }
.stat-plain{ background: linear-gradient(160deg, rgba(40,36,32,0.7), rgba(10,9,8,0.75)); }
...
/* ---------- CARD PANEL ---------- */
.panel{
  background: linear-gradient(175deg, rgba(30,26,38,0.55), var(--glass-dark) 60%);
...
.answer-card{
  ...
  background: linear-gradient(155deg, rgba(255,90,31,0.20) 0%, rgba(157,111,255,0.14) 55%, rgba(10,10,10,0.62) 100%);
...
/* ---------- HISTORY CARDS ---------- */
.hist-card{
  border: 1px solid var(--card-border); background: linear-gradient(165deg, rgba(28,24,34,0.6), var(--glass-dark) 65%);
  border-radius: 18px;
...
.loader-badge{
  position:relative; width:min(300px, 62vw); height:min(300px, 62vw);
...
.loader-ring-progress{
  ...
  filter: drop-shadow(0 0 6px rgba(255,154,82,0.95)) drop-shadow(0 0 16px rgba(255,90,31,0.55));
...
.loader-ring-head{
  ...
  box-shadow: 0 0 10px 4px rgba(255,154,82,0.9), 0 0 22px 9px rgba(255,90,31,0.45);
}
...
def _generate_plexus_svg(seed=42, num_points=150, width=1600, height=900, connect_dist=165):
    ...
    for i in range(num_points):
        for j in range(i + 1, num_points):
            ...
            if d < connect_dist:
                op = max(0.03, 0.32 * (1 - d / connect_dist))
                ...
                lines.append(
                    ...
                    f'stroke="rgba(150,200,255,{op:.3f})" stroke-width="1"{pulse_style}/>'
                )
...
<div class="bg-video-wrap">
  <div class="bg-plexus-wrap">{_generate_plexus_svg()}</div>
...
if st.session_state.app_stage == "loading":
    ...
    # ============================================================
    st.markdown(
        '<div class="app-footer reveal reveal-5 scramble">AI Financial Voice Assistant · Powered by Gemini</div>',
        unsafe_allow_html=True,
    )
    # No per-stage trigger needed here — the single persistent scramble
    # iframe injected near the top of the file (see _SCRAMBLE_SCRIPT_HTML)
    # watches the whole page for the life of the session and picks up
    # this footer, the answer card, updated stat counts, etc. on its own.
