...
/* a dark core fading up from the bottom, like the reference fintech card —
   sits between the colour gradient and the content so every card reads as
   glass with real depth rather than a flat tint */
.stat-card::before, .panel::before, .answer-card::before, .hist-card::before{
  content:''; position:absolute; inset:0; border-radius:inherit; z-index:0; pointer-events:none;
  background: radial-gradient(120% 90% at 50% 115%, rgba(0,0,0,0.62) 0%, rgba(0,0,0,0.28) 42%, rgba(0,0,0,0) 72%);
}
.stat-card > *, .panel > *, .answer-card > *, .hist-card > *{ position:relative; z-index:1; }
.stat-card .label{
  font-size: 0.72rem; color: rgba(255,255,255,0.82); font-weight:700;
  letter-spacing:0.08em; text-transform:uppercase;
}
.stat-card .value{
  font-size: 2.15rem; font-weight: 800; color: #fff; margin-top: 13px;
  font-family:'JetBrains Mono', monospace; letter-spacing: 0.03em;
  text-shadow: 0 0 18px rgba(255,255,255,0.35), 0 2px 10px rgba(0,0,0,0.35);
  font-variant-numeric: tabular-nums;
}
.stat-card .sub{ font-size: 0.72rem; color: rgba(255,255,255,0.72); margin-top:6px; letter-spacing:0.01em; }
...
.stat-glass-orange{ background: linear-gradient(160deg, rgba(255,122,61,0.85), rgba(255,90,31,0.72) 55%, rgba(80,20,60,0.60)); border-color: rgba(255,154,82,0.35); }
.stat-glass-white{ background: linear-gradient(160deg, rgba(61,127,255,0.85) 0%, rgba(14,22,50,0.80) 100%); border-color: rgba(120,160,255,0.32); }
.stat-glass-black{ background: linear-gradient(160deg, rgba(157,111,255,0.85) 0%, rgba(28,17,50,0.80) 100%); border-color: rgba(180,140,255,0.30); }
.stat-plain{ background: linear-gradient(160deg, rgba(50,45,40,0.68), rgba(8,7,6,0.72)); }
.stat-plain .value{ color: var(--text); text-shadow: 0 0 14px rgba(255,255,255,0.14), 0 2px 10px rgba(0,0,0,0.4); }
...
.dotgrid{
  ...
  background-size: 9px 9px; opacity:0.5; z-index:1;
}

/* ---------- VOICE CONSOLE (Siri-style wave + mic) ---------- */
.voice-console{
  position:relative; z-index:1; display:flex; flex-direction:column; align-items:center;
  gap: 28px; margin-top: 56px;
}
.wave-canvas-wrap{ width: min(720px, 90vw); height: 140px; position:relative; }
#voice-wave-canvas{ width:100%; height:100%; display:block; }
...
.voice-mic-btn .mic-core{
  position:relative; width:100%; height:100%; border-radius:50%;
  background: linear-gradient(160deg, var(--orange-b), var(--orange-a));
  box-shadow: 0 10px 30px rgba(255,90,31,0.38), 0 1px 0 rgba(255,255,255,0.25) inset;
  display:flex; align-items:center; justify-content:center;
  transition: transform 0.22s var(--ease), box-shadow 0.22s var(--ease);
}
.voice-mic-btn:hover .mic-core{ transform: translateY(-2px) scale(1.03); }
.voice-mic-btn .mic-core svg{ width:28px; height:28px; fill:#fff; }
.voice-mic-btn .mic-ring{
  position:absolute; inset:-8px; border-radius:50%;
  border: 2px solid rgba(255,90,31,0.45); opacity:0; transform: scale(0.85);
}
.voice-mic-btn.listening .mic-core{
  box-shadow: 0 10px 34px rgba(255,90,31,0.55), 0 1px 0 rgba(255,255,255,0.3) inset;
}
...
.voice-status{
  font-size: 0.8rem; color: var(--text-onbg-dim); letter-spacing:0.03em; min-height: 1.2em;
}

/* ---------- VOICE ANSWER — translucent orange glass bar ---------- */
.voice-answer-bar{
  width: min(760px, 92vw); margin: 6px auto 0 auto; padding: 20px 26px;
  border-radius: var(--radius);
  background: linear-gradient(155deg, rgba(255,122,61,0.30), rgba(255,90,31,0.16) 55%, rgba(10,10,10,0.30));
  border: 1px solid rgba(255,154,82,0.35);
  backdrop-filter: blur(var(--blur)) saturate(160%); -webkit-backdrop-filter: blur(var(--blur)) saturate(160%);
  box-shadow: 0 1px 0 rgba(255,255,255,0.10) inset, 0 18px 40px rgba(0,0,0,0.18);
  color: #1a1512; font-size: 0.98rem; line-height:1.6; white-space: pre-wrap;
  animation: fadeRise var(--dur) var(--ease) both;
}
.voice-answer-bar .voice-answer-tag{
  display:block; font-size:0.72rem; font-weight:700; color: var(--orange-b);
  text-transform:uppercase; letter-spacing:0.06em; margin-bottom:8px;
}

/* hidden bridge input — JS writes the recognized speech into this
   Streamlit text_input so Python can pick it up on the next rerun;
   kept in the DOM (not display:none) so it stays focusable/dispatchable */
.st-key-voice_transcript_wrap{
...
if st.session_state.app_stage == "loading":
    ...
    # Pro first: complex, multi-step financial questions (amortization,
    # multi-scenario comparisons, tax/investment reasoning) benefit far more
    # from Pro's stronger reasoning than Flash's speed. Flash and older models
    # remain as fallbacks if Pro isn't available on this API key.
    MODEL_PREFERENCES = [
        "gemini-2.5-pro",
        "gemini-2.5-flash",
        "gemini-2.0-flash",
    ...
    # ------------------------------------------------------------
    # Dedicated reasoning persona for the voice assistant. Kept separate
    # from the tiny probe model above so the "ping" health-check never
    # pays for this much system-prompt context, and so it's easy to tune
    # independently of model discovery.
    # ------------------------------------------------------------
    VOICE_SYSTEM_INSTRUCTION = """You are the reasoning engine behind a voice-driven financial assistant. Users ask complex, real-world financial questions out loud — loan and mortgage comparisons, amortization, compound interest, retirement and savings planning, currency conversion, tax estimates, investment return scenarios, budgeting, business finance ratios, and multi-part "what if" questions.

For every question:
1. Work through the problem step by step internally and double-check your arithmetic before answering — get the numbers right, this is a calculator people rely on.
2. If a detail is missing or ambiguous, make the most reasonable real-world assumption, state it in one short clause, and answer anyway — the user is speaking and can't reply to a clarifying question.
3. If the question has multiple parts or asks you to compare scenarios, address every part clearly.
4. If the question isn't actually financial, still answer it directly and helpfully — don't refuse.

Formatting rules — your answer is shown as plain text in a small glass card, not rendered markdown or LaTeX:
- Never use markdown symbols such as **, *, #, or backticks, and never use LaTeX such as $, \\(, \\[, \\frac, or \\times.
- Write math in plain notation: "500 x 24 = 12,000", "8% annual return", "$1,240.50".
- Prefer short "Label: value" lines over long paragraphs or tables.
- Keep the whole answer tight and skimmable: a few short lines plus one concluding takeaway sentence, unless the question genuinely needs more depth."""

    @st.cache_resource(show_spinner=False)
    def build_answer_model(name):
        """Builds the real answer-generating model with the financial-reasoning
        system prompt attached. Separate from find_working_model's bare probe
        instance because system_instruction must be set at construction time."""
        if not name:
            return None
        try:
            return genai.GenerativeModel(name, system_instruction=VOICE_SYSTEM_INSTRUCTION)
        except Exception:
            return None

    answer_model = build_answer_model(model_name)

    _LATEX_FRAC_RE = re.compile(r"\\frac\{([^{}]*)\}\{([^{}]*)\}")

    def clean_answer_text(text):
        """Safety net that strips markdown/LaTeX artifacts the model might
        still slip in, since the answer renders as plain pre-wrap text, not
        through a markdown/KaTeX pipeline."""
        if not text:
            return text
        t = text
        ...
        t = re.sub(r"\$\$(.*?)\$\$", r"\1", t, flags=re.DOTALL)
        t = re.sub(r"\$(.*?)\$", r"\1", t, flags=re.DOTALL)
        t = _LATEX_FRAC_RE.sub(r"(\1)/(\2)", t)
        t = t.replace("\\times", "x").replace("\\cdot", "x").replace("\\div", "/")
        t = t.replace("\\%", "%").replace("\\approx", "≈")
    ...
    # ============================================================
    # 8. VOICE CONSOLE — Siri-style reactive wave + mic button.
    # The wave idles with a gentle flowing motion always, and while the
    # mic is listening it reacts in real time to actual microphone
    # volume (Web Audio API), not a canned animation. Speech is
    # transcribed in-browser (Web Speech API) and handed to Python
    # through a visually-hidden st.text_input bridge so the existing
    # Gemini call can run normally and the answer renders below in the
    # translucent orange glass bar.
    # ============================================================
    ...
    st.markdown("""
    <div class="voice-console reveal reveal-3">
      <div class="wave-canvas-wrap"><canvas id="voice-wave-canvas"></canvas></div>
      <button type="button" class="voice-mic-btn" id="voice-mic-btn">
        <span class="mic-ring"></span>
        <span class="mic-core">
          <svg viewBox="0 0 24 24"><path d="M12 14a3 3 0 0 0 3-3V5a3 3 0 0 0-6 0v6a3 3 0 0 0 3 3zm5-3a5 5 0 0 1-10 0H5a7 7 0 0 0 6 6.92V21h2v-3.08A7 7 0 0 0 19 11h-2z"/></svg>
        </span>
      </button>
      <div class="voice-status" id="voice-status">Tap the mic and ask a financial question</div>
    </div>
    """, unsafe_allow_html=True)

    with st.container(key="voice_transcript_wrap"):
        st.text_input("voice transcript bridge", key="voice_transcript_value", label_visibility="collapsed")

    transcript = st.session_state.get("voice_transcript_value", "")
    if transcript and transcript != st.session_state.voice_last_transcript:
        st.session_state.voice_last_transcript = transcript
        active_model = answer_model or model
        if active_model:
            with st.spinner("AI analyzing…"):
                try:
                    ...
                    cursor.execute(
                        ...
                        ("Voice", transcript, answer, datetime.now().strftime("%b %d, %Y · %H:%M")),
                    )
                    conn.commit()
                except Exception as e:
                    st.session_state.voice_answer = f"AI Error: {e}"
        else:
            st.session_state.voice_answer = "The AI model isn't connected yet — check your GEMINI_API_KEY in Secrets."

    if st.session_state.voice_answer:
        st.markdown(f"""
        <div class="voice-answer-bar">
          <span class="voice-answer-tag">Answer</span>{st.session_state.voice_answer}
        </div>
        """, unsafe_allow_html=True)
    ...
    (function(){
      try {
        ...
        function boot(){
          var canvas = doc.getElementById('voice-wave-canvas');
          var btn = doc.getElementById('voice-mic-btn');
          var status = doc.getElementById('voice-status');
          if (!canvas || !btn || !status) { win.setTimeout(boot, 200); return; }
          if (canvas.dataset.voiceWaveRunning === '1') return;
          canvas.dataset.voiceWaveRunning = '1';
          ...
          var listening = false;
          var audioCtx = null, analyser = null, dataArray = null, micStream = null;
          var level = 0; // smoothed 0..1 mic level
          var idleT = 0;
          ...
          function drawWave(ts){
            ...
            var target = listening ? level : 0;
            // amplitude: a calm idle ripple even at rest, a much bigger one while listening
            var baseAmp = 3 + target * (H * 0.42);
            var midY = H / 2;
            var layers = [
              { amp: baseAmp, freq: 0.018, speed: 1.0, color: 'rgba(255,90,31,0.85)', width: 2.2 },
              { amp: baseAmp * 0.7, freq: 0.026, speed: 1.6, color: 'rgba(45,52,68,0.5)', width: 1.4 },
              { amp: baseAmp * 0.45, freq: 0.034, speed: 2.2, color: 'rgba(45,52,68,0.28)', width: 1.2 }
            ];
            ...
            for (var l = 0; l < layers.length; l++){
              ...
              ctx2d.beginPath();
              for (var x = 0; x <= W; x += 4){
                var idle = Math.sin(x * 0.02 + idleT * 1.4) * 2.5;
                var y = midY
                  + Math.sin(x * layer.freq + idleT * layer.speed) * layer.amp
                  * Math.sin((x / W) * Math.PI) // taper to flat at both ends, like the reference
                  + idle;
            ...
            win.requestAnimationFrame(drawWave);
          }
          win.requestAnimationFrame(drawWave);
          ...
          function sampleLevel(){
            ...
            level = level * 0.7 + Math.min(1, rms * 4) * 0.3; // smoothed
            if (listening) win.requestAnimationFrame(sampleLevel);
          }

          function setBridgeValue(text){
            try {
              var input = doc.querySelector('.st-key-voice_transcript_wrap input');
              if (!input) return;
          ...
          var SpeechRec = win.SpeechRecognition || win.webkitSpeechRecognition;
          ...
          function startListening(){
            ...
            navigator.mediaDevices.getUserMedia({ audio: true }).then(function(stream){
              ...
              win.requestAnimationFrame(sampleLevel);
            }).catch(function(){
            ...
            if (SpeechRec){
              ...
              recognition.onerror = function(){
                status.textContent = 'Could not hear that — try again.';
              };
              recognition.onend = function(){ stopListening(); };
              recognition.start();
            } else {
              status.textContent = 'Voice recognition is not supported in this browser — try Chrome.';
            }
          }
          ...
          function stopListening(){
            ...
            if (status.textContent === 'Listening…') status.textContent = 'Tap the mic and ask a financial question';
    ...
    """
    components.html(_VOICE_WAVE_HTML, height=1, scrolling=False)

    # Everything else below removed per request — the dashboard now shows
    # only the title, the four stat cards, and the voice console above.
