...
@media (prefers-reduced-motion: reduce){ .bg-plexus{ animation: none !important; } }
.bg-tint{
...
def _generate_plexus_svg(seed=42, num_points=150, width=1600, height=900, connect_dist=165):
        ...
        for j in range(i + 1, num_points):
            ...
            if d < connect_dist:
                ...
                lines.append(
                    ...
                    f'stroke="rgba(150,200,255,{op:.3f})" stroke-width="1"/>'
                )
    ...
    for (x, y) in pts:
        ...
        dots.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="{r:.2f}" fill="rgba(205,230,255,{op:.2f})"/>')
