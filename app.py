...
.loader-ring{
  ...
  border-top-color: var(--orange-a);
  animation: bg-spin 0.85s linear infinite;
}
@keyframes bg-spin{ to{ transform: rotate(360deg); } }
.loading-brand{
  font-family: 'Jura', 'Inter', -apple-system, sans-serif;
  font-size: 1.3rem; font-weight: 700; color: var(--text); letter-spacing: 0.02em;
}
.loading-text{
...
/* ---------- WELCOME SCREEN ---------- */
/* When the welcome title is present, turn the whole block-container into a
   centered flex column so the title and the button below it are grouped
   and centered together on screen (scoped to this stage only via :has()). */
.block-container:has(.welcome-title){
  min-height: 82vh;
...
.st-key-begin_wrap{ display:flex; justify-content:center; margin-top: 12px; }
.st-key-begin_wrap .stButton{ display:flex; justify-content:center; }
...
if st.session_state.app_stage == "loading":
    ...
    <div class="loading-wrap">
      ...
      <div class="loading-brand">AI Financial Voice Assistant</div>
      <div class="loading-text">Loading<span class="loading-dots"><span>.</span><span>.</span><span>.</span></span></div>
    </div>
    """, unsafe_allow_html=True)
    time.sleep(1.6)
